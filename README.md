...
- 快速分类会为当前视频自动按需生成 H.264 浏览器兼容预览，并预缓存下一条；兼容缓存只用于网页播放，归档的仍是原视频。
- 工具不会覆盖内容不同的同名文件。
...
- 工具会从项目目录向上查找最近的 `candidates.csv`；当 CSV 含有 `file` 和 `device_id` 列时，会按视频文件名自动关联真实设备 ID。即使视频文件名本身没有设备 ID，也能查询、分组和导航。
...
- CSV 映射优先级最高。找不到 CSV 或个别视频未匹配时，才会尝试从重复文件名字段自动识别；也可在“识别规则”中配置分隔字段或正则表达式。
