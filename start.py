...
def choose_with_gui() -> dict[str, str] | None:
    ...
    def submit() -> None:
        ...
        if not output_var.get().strip() or not no_fall_var.get().strip() or not fall_var.get().strip():
        ...
        active_outputs = [selection["fall_output"]] if selection["mode"] == "label" else [selection["output"], selection["no_fall_output"]]
...
def main() -> int:
    ...
    if mode == "label":
        ...
        sys.argv = [str(Path(quick_labeler.__file__).resolve()), "--source", str(source), "--output", str(fall_output), "--host", args.host, "--port", str(args.port)]
